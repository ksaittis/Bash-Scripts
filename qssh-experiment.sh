...
PRIMARY_PRIVATE_KEY_NAME=${USERNAME}.pem
...
execute_ssh_command() #executes command to remote server through ssh and returns result
{
...
clean_ssh_command() #Removes double quotes from variable so it can be used as command
{
...
#unreadable code
identify_arguments_passed() #used to identify positional parameters passed to script
{ #parameters must not be passed in the correct order
    if [[ $# -eq 2 ]]; then #check number of positional parameters passed
        i=0
        for arg in $@; do
            if [[ $arg =~ ^- ]]; then #identify which positional parameter is related to account
                source ./change-aws.sh
                eval account_option='$'$((i+1))
                if [[ $((i+1)) == 1 ]]; then #identify which positional parameter is related to NAME_TAG
                    NAME_TAG=$2
                else
                    NAME_TAG=$1
                fi
    ...
    elif [[ $# -eq 1 ]]; then #if one atgument was passed
        if [[ $1 =~ ^- ]]; then #identify if the positional parameter is related to account
            source ./change-aws.sh
            account_option=$1
            if [[ $account_option == -p ]]; then
                /bin/bash change-aws prod
...
connect_to() #handles the connection to the instance
{
...
display_help_msg() #displays help message
{
cat << EOF
...
__main__() #defines the order in which the functions are being called
{
...
#method called with positional parameters
__main__ $@
