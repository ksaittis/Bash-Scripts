...
display_wars_versions() #displays the war versions found on that intance
{
	...
	makeline -
    centerPrint "WAR VERSION ON INSTANCE ${instance_name^^}"
    makeline -
    for i in ${!WARS[@]}; do
...
display_unsuccessful_login_msg() #display Unsuccessful login message
{
    ...
    makeline -
    centerPrint ${CYAN}${BOLD}"POSSIBLE SCENARIOS"${NORMAL}
    centerPrint ${CYAN}"Wrong username or key used."
...
{ #parameters must not be passed in the correct order
    if [[ $# -eq 2 ]]; then #check number of positional parameters passed
        ...
        centerPrint "One for name_tag and one for account type [-p|-np]."${NORMAL}
        kill -INT $$
    fi
}

tryDifferentCredentials()
{
    centerPrint ${YELLOW}${BOLD}"USERNAME: $USERNAME KEY: $KEY didn't work."${NORMAL}
    centerPrint ${YELLOW}${BOLD}"Trying different combination"${NORMAL}
    if [[ "$USERNAME" == "ec2-user" ]]; then #using non prod credentials into prod
        USERNAME=$PRIMARY_USERNAME
        KEY=$PRIMARY_PRIVATE_KEY_NAME
    elif [[ "$USERNAME" == "vtw-dev" ]]; then #using prod credentials into non prod
        USERNAME=$BACKUP_USERNAME
        KEY=$BACKUP_KEY_NAME
    fi
...
connect_to()
{
    ...
    if [[ "$instance_number" =~ $re && "$instance_number" -le $((${#INSTANCES[@]} -3 )) ]]; then
        ...
        if [[ -z ${sshcommand} ]]; then
            ...
            ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 -i ~/.ssh/${KEY} ${USERNAME}@${instance_ip//[[:space:]]/}
            if [[ $? -eq 255 ]]; then #if it fails once try different credentials
                tryDifferentCredentials
                if [[ $? -eq 255 ]]; then #if it fails twice then display_unsuccessful_login_msg
                    display_unsuccessful_login_msg
                else
                    kill -INT $$ #else if the second attemp succeded then quit
                fi
            fi
        else
        	clean_ssh_command
...
kill -INT $$ #kill after displaying the message
}
